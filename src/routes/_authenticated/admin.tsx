...
function AdminHome() {
  ...
  const counts = useQuery({ queryKey: ["admin-counts"], enabled: isAdmin, queryFn: async () => {
    const tables = ["profiles", "questions", "current_affairs", "syllabus_topics", "resources", "mains_questions"] as const;
  ...
  const links = [["Users", "/admin/users"], ["Questions", "/admin/questions"], ["Current Affairs", "/admin/current-affairs"], ["Syllabus", "/admin/syllabus"], ["Resources", "/admin/resources"], ["Mains", "/admin/mains"]] as const;
