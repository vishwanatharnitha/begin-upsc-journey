...
function SubjectDetail() {
  ...
  const resources = useQuery({ queryKey: ["subject-resources", id], queryFn: async () => { const { data, error } = await supabase.from("resources").select("id, title, resource_type, description, external_url, file_url").eq("subject_id", id).eq("published", true).limit(12); if (error) throw error; return data ?? []; }});
